...
var b2 = (function () {
  ...
  contactListener.PostSolve = function (contact, impulse) {
    ...
    var damage = function (bodyData) {
      if (
        !bodyData ||
        (bodyData.getHealth() == bodyData.getFullHealth() && imp0 < 12)
      )
        return;
  ...
  return {
    ...
    enablePhysicsFor: function (desc) {
      var bodyDef = new b2BodyDef(),
        ...
        center = cc.p(
          position.x - anchPoint.x + size.width / 2,
      ...
      switch (desc.shape) {
        case "circle":
          fixDef.shape = new b2CircleShape(
            toWorld(desc.radius || size.height / 2)
          );
          break;
        case "box":
          fixDef.shape = new b2PolygonShape();
          fixDef.shape.SetAsBox(
            toWorld(size.width) / 2,
            toWorld(size.height) / 2
          );
          break;
      }
